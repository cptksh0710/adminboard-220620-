const Countrycode = require("../models/countrycode.model.js");
...
exports.create = (req,res)=>{
    ...
    // 데이터베이스에 저장
    Countrycode.create(countrycode, (err, data) =>{
        if(err){
            res.status(500).send({
                message:
            ...
            res.send('<script type="text/javascript"> alert("코드가 등록되었습니다"); window.location="/countrycode"; </script>');
        };
    })
};
...
exports.findOne = (req,res)=>{
  Countrycode.findById(req.params.code_idx, (err, data) => {
        if (err) {
...
exports.update = (req,res)=>{
  ...
  Countrycode.updateById(
    req.params.code_idx,
    new Countrycode(req.body),
    (err, data) => {
      if (err) {
        if (err.kind === "찾을 수 없습니다.") {
          res.status(404).send({
...
exports.delete = (req,res)=>{
  Countrycode.remove(req.params.code_idx, (err, data) => {
        if (err) {
          if (err.kind === "찾을 수 없습니다") {
            res.status(404).send({
              message: `Not found code ${req.params.code_idx}.`
          ...
          res.send('<script type="text/javascript"> alert("코드가 삭제되었습니다"); window.location="/countrycode"; </script>');
        }
      });
};
...
exports.deleteAll = (req,res)=>{
  Countrycode.removeAll((err, data) => {
        if (err)
          res.status(500).send({
            message:
